...
export default function TitleBar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 h-10 z-50 flex items-stretch select-none" style={{ WebkitAppRegion: 'drag' } as CSSProperties}>
      <div className="flex items-center px-3 gap-2 text-zinc-400 text-xs">
        <span className="w-3.5 h-3.5 rounded-[5px] bg-gradient-to-br from-indigo-500 to-fuchsia-500 inline-block shadow-[0_0_8px_rgba(129,90,246,.45)]" />
        <span className="font-medium tracking-wide text-zinc-300">File Organizer</span>
      </div>
      ...
      <div className="flex items-stretch" style={{ WebkitAppRegion: 'no-drag' } as CSSProperties}>
        <button
          ...
          className="w-11 flex items-center justify-center text-zinc-400 hover:text-zinc-100 hover:bg-white/[.07] transition-colors"
        >
          <WindowIcon d="M1 5h8" />
        </button>
        <button
