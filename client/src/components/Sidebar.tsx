    ...
    <aside className="relative z-40 flex flex-col shrink-0 w-[76px] lg:w-60 border-r border-[var(--border)] bg-[var(--bg-2-glass)] backdrop-blur-xl transition-[width] duration-200">
      ...
      <button onClick={onHome} className="flex items-center gap-3 h-16 px-3 lg:px-4 group shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]">
        ...
        <div className="hidden lg:block leading-tight text-left">
          ...
          <div className="text-[11px] text-[var(--text-3)]">{t('Suite Tools Lokal')}</div>
        </div>
      </button>

      {/* Nav */}
      <nav className="flex-1 overflow-y-auto scrollbar-none px-3 lg:px-3 pt-3 space-y-1.5">
        <div className="hidden lg:block eyebrow px-3 pb-2.5">{t('Tools')}</div>
        ...
        >
          ...
          <span className="hidden lg:inline">{t('Beranda')}</span>
        </button>
        ...
        {ACTIVE_TOOLS.map((tool) => (
          <button
            ...
            title={t(tool.short)}
            className={`nav-item ${current === tool.tool ? 'active' : ''}`}
          >
            <span className="nav-icon">
              <Icon name={tool.icon} className="w-4 h-4" />
            </span>
            <span className="hidden lg:inline truncate">{t(tool.short)}</span>
          </button>
        ))}
      </nav>

      {/* Footer */}
      <div className="shrink-0 px-3 lg:px-4 py-4 border-t border-[var(--border)] space-y-1">
        <button
        ...
        >
          ...
          <span className="hidden lg:inline">{t('Ganti Tema')}</span>
        </button>
        <button
          ...
          className="w-full flex items-center gap-2.5 lg:gap-3 h-11 rounded-xl px-3 text-[13px] font-medium text-[var(--text-2)] hover:bg-[var(--overlay)] hover:text-[var(--text)] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)]"
        >
          <span className="grid place-items-center w-8 h-8 rounded-lg bg-[var(--overlay-2)] text-[var(--text-2)] shadow-sm">
            <Icon name="globe" className="w-4 h-4" />
          </span>
          <span className="hidden lg:inline">{t('Ganti Bahasa')}</span>
        </button>
